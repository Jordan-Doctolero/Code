...
def pipe_movement(pipes, speed):
    new_pipes = []
    for pipe in pipes:
        pipe["rect"].centerx = int(pipe["rect"].centerx - speed)
...
#Clears all pipes from the list
def delete_all_pipes(screen, pipes):
    pipes.clear()

#This spawns the boxes at given width and random height as well as a random velocity 
...
#This handles the movements of the boxes including bouncing off the top and bottom of the screen
...
#This is what actually draws the boxes to the screen
def box_creation(screen, boxes, color):
    for b in boxes:
        pygame.draw.rect(screen, Black, b)

#Clears all boxes from the list
def delete_all_boxes(screen, boxes):
    boxes.clear()

# Triangles
#This spawns a triangle at the right edge of the screen with random vertical position with a given veloctiy at random
def triangle_spawner(screen, tsize):
    ...
    x_velocity = random.randint(-25,-15)

    return [points, x_velocity]

#This moves the triangle to the left at a given speed
...
#This is what actually draws the triangles to the screen
def triangle_creation(screen, triangles, color):
    for tri in triangles:
        points, _ = tri
        pygame.draw.polygon(screen, color, points)

#This checks if the triangle is off the screen
def triangle_off_screen(points):
    xs = [p[0] for p in points]
    return max(xs) < 0

#This checks for collision between the bird and triangles using the same lives + immunity system as pipes
def check_triangle_collision(bird, triangles, state):
    now = time.time()
...
def draw_item(screen, rect, color, born):
    ...
    pygame.draw.circle(glow, (*coin_colors[color], 80), (rect.width // 2 + 15, rect.height // 2 + 15), int(pulse + 10))
    screen.blit(glow, (rect.centerx - rect.width // 2 - 15, rect.centery - rect.height // 2 - 15))
...
# -----------------------------------
#Jordan had to update to include cycle count for scoring purposes
def handle_item_collision(items, bird, state,cycle_count):
    updated = []
    now = time.time()
